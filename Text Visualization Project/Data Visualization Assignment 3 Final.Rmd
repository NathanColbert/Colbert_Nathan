...
#set.seed(12345)
#thou_suc <- KSDS %>% arrange(desc(Achievement_Ratio)) %>% head(n = 1000)

#tenthou_bad <- KSDS %>% arrange(Achievement_Ratio) %>% head(n = 10000) %>% as.tbl()
#thou_bad <- sample_n(tenthou_bad, size = 1000)

#suc_story <- rbind(thou_suc, thou_bad)
#write.csv(suc_story, "success_story2.csv")
```
